...
const fetchUserDetail = async (id: string) => {
  const response = await fetch(`/api/dashboard/superadmin/users/detail?id=${id}`)
  if (!response.ok) {
...
const InfoItem = ({ icon, label, value }: { icon: React.ReactNode, label: string, value: string }) => (
  <div className="flex items-center p-3 rounded-lg">
    <div className="flex-shrink-0 w-9 h-9 bg-indigo-100 rounded-full flex items-center justify-center mr-3">
      {icon}
    </div>
...
export default function UsersDetail({ user, onClose }: { user: UserDetail, onClose: () => void }) {
  const { data: userDetail } = useSuspenseQuery<UserDetail>({
  ...
  const userInfoItems = [
    {
      ...
      content: (
        <div className="flex flex-col gap-4">
          <InfoItem icon={<User className="h-4 w-4" />} label="Name" value={userDetail.name} />
          <InfoItem icon={<Mail className="h-4 w-4" />} label="Email" value={userDetail.email} />
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        <Avatar className="h-16 w-16">
          <AvatarImage src={userDetail.imageFile} alt={userDetail.name} />
        ...
        <div className="flex flex-col gap-1">
          ...
          <p className="text-sm text-muted-foreground">User ID: {userDetail.userId}</p>
        </div>
      </div>
      ...
      <>
        ...
        <div className='overflow-hidden border-t pt-[.65rem] border-zinc-200 dark:border-zinc-700'>
          <TransitionPanel
